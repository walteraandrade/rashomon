...
2. For every numbered acceptance criterion that a unit test can check, write one test whose name quotes the criterion number, e.g. `it('AC3: unknown kind falls back to all')`. Extend the fixture when needed; keep it small and deterministic.
   - **Tests go into the module's file, never into a new `<issue>-acceptance.test.ts`.** `test/` holds one file per `src` module or figure (`graph.test.ts`, `query.test.ts`, `store.test.ts`, `collectors-<name>.test.ts`, `figures-<name>.test.ts`, `render.test.ts`, ...; see `docs/factory.md`). Add a `describe` titled with the issue number to the file of the module the criterion exercises; a criterion about a route goes in `server.test.ts`, one about documented facts in `docs-drift.test.ts`. If you write a suite that inserts past the fixture, end it with `after(reseed)`.
