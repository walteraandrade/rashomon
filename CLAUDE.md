...
- `public/js/app.js` is a shell: it fetches `/api/people` once, seeds each figure from bare and prefixed querystring keys, and calls each figure's own `mount()`. Its only export is `boot`; `boot()` runs only when a `document` exists, which is what lets tests import the wiring.
