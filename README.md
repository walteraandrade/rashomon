...
Returns `{ person, stats, nodes, links, signature, outlets }`. Each node also carries `tone`: average GDELT tone of the docs where the term co-occurs with the person, null when no GDELT doc contributed. `pmi` is log2 of how much more often the term co-occurs with the person than chance, measured against every collected doc in the window. `outlets` is `{ domain, lean, basis }[]`, empty when `lean` is unset/`all`, otherwise every `outlets.json` entry matching the resolved `domain`+`lean` scope.
...
- `public/design-5.html` current UI (radial atlas), served at `/`; markup only, plus one `<script type="module">` that wires DOM events to the modules below. `public/atlas.css` holds every style rule (tokens, layout, components); `public/index.html` legacy UI
- `public/js/api.js` network layer: builds API URLs and fetches (`endpoint`, `params`, `json`, `loadPeople`, `loadGraph`, `loadSources`, `loadDocs`)
- `public/js/layout.js` pure word-packing/edge-routing geometry (`wrapLines`, `centerLabel`, `packPass`, `pack`, `routeGraph`, `routesFrom`); no DOM — text metrics come in as an injected `measure(text, size, family, weight)` function, satisfied at runtime by `render.js`'s `createCanvasMeasure()` (the one place that touches a `<canvas>`)
- `public/js/render.js` DOM layer: paints the map, inspector, outlets and docs panel (`drawMap`, `paintSelection`, `inspect`, `paintColumns`, `wordMarkup`, `paintOutlets`, `paintDocs`, …); reads/writes the document, never fetches
- `public/js/state.js` mutable UI state owned in one place: `state` (source/domain filters), selection, zoom, the layout cache, and request/abort bookkeeping
- `public/js/format.js` pure formatting/label helpers shared by the modules above (`esc`, `fmt`, `label`, `normalize`, `score`, `scoreName`, `sourceLabels`, `bskyUrl`, `safeDocUrl`, …)
- `docs/designs/` archived design alternatives (`design-1..4/6.html`, `graph-lab.html`, `graph-circle-lab.html`, `designs.html`) kept for reference only; not under `public/`, so the static file server never reaches them
- `test/` node:test suites; `test/fixture.ts` seeds the in-memory database; `test/layout.test.ts`, `test/format.test.ts`, `test/api.test.ts` and `test/bsky-link.test.ts` import the `public/js/*.js` modules directly rather than reading `design-5.html` as text
