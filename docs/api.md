...
`GET /api/people/:id/lenses?a=<lens>&b=<lens>&days=30&kind=all&limit=40` shows one tracked person's terms under two independently-scoped lenses — read through two different outlets, two political leans, or two sources — on the union of their strongest terms, the flip of `/compare` (two scopes, one person, instead of one scope, two people). Nested under `/people/:id/*`: an unknown `:id` returns the same 404 `{ "error": "person not found" }` every other route in that family uses.

A lens is one of `domain:<host>`, `lean:<left|right|center>` or `source:<name>`. `<host>` only needs to match the domain token charset `/graph?domain=` already checks — this route never validates it against the person's actual outlets, so an unmatched host simply returns zero docs on that side. `lean:` takes exactly one value, not a list. A missing parameter, an unparseable value, or an unknown prefix all fall back to `all` — the full corpus for that side, scoped only by `days`/`kind` — and this is never an error: the response's own `a.lens`/`b.lens` echoes back the normalized token actually used (the literal string `"all"` on a fallback), never the malformed input, so a caller can tell the fallback happened. `days` and `limit` snap the same way `/compare`'s do (`limit` capped at `SMALL_LIMITS`, 100); there is no `min`, `sort`, `domain`, `lean`, `source` or `country` parameter at the top level — those live inside `a`/`b` instead, and `country` is always `br`.

Returns `{ days, a, b, terms }`. `a`/`b` are each `{ lens, about }`: `lens` the normalized token, `about` that side's own doc count for the person under that lens (the same meaning `/graph`'s `stats.about` carries). `terms` holds one row per unioned `(term, kind)` key, `{ term, kind, a, b }`: each side's value is `{ count, pmi, tone }`, `null` (measured: the term never appears on that side, in scope), or the string `"name"` — always the same on both sides, since both lenses describe the same person and the same `nameTokens` exclude set. Unlike `/compare`, the two sides never share one PMI denominator: each lens can cover a wildly different slice of the corpus, so each gets its own `n`/`term_all`. `a === b` (the same normalized lens on both sides) is legal and returns identical figures on both sides. No aggregate table backs this route — every request runs the live statement, at every window, with no fast path even for a `source:` lens.
