...
Every collector runs on Effect's fetch-backed `HttpClient` (`src/http.ts`, `fetchClient`): `getBytes` is the one capped, timed GET every collector's request goes through, so every collector's network read is timeout-bounded, not only `gdelt`/`camara`/`senado`/`bluesky`: this layer covers `rss` and `gkg` too, which had no ceiling before it. The 45 s ceiling (`REQUEST_TIMEOUT_MS`) is `getBytes`'s default third argument, and covers the whole request, first byte sent to last byte read, where a per-socket idle timer would instead wait through a body that merely trickles. `gkg.ts`'s zip download is the one exception: a slot measures 13-14MB compressed, too large for a 45 s ceiling sized for small JSON endpoints, so `download` passes its own `GKG_DOWNLOAD_TIMEOUT_MS` (300 000 ms, 5 min) as `getBytes`'s third argument instead — a ceiling distinct from both `REQUEST_TIMEOUT_MS` and the `MAX_RESPONSE_BYTES`/`MAX_EXPANDED_BYTES` byte caps, all three enforced independently. Effect's client also propagates trace context (`traceparent`, `b3`) by default; `fetchClient` turns it off, so the only request header is still the project `user-agent` (`test/http.test.ts` pins the exact header set). Every module additionally exports its Effect as `collect`, a seam for tests only (`test/effect.ts`'s `fakeFetch`/`runTest`/`drain`/`TestClock`/`TestConsole`); `src/collectors/index.ts` and `server.ts` still see only the Promise `Collector` each module exports alongside it.
