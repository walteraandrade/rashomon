...
| `DATABASE_URL` / `POSTGRES_URL` | unset | switches every command's `SqlClient` from embedded PGlite (`@effect/sql-pglite`) to a managed Postgres (`@effect/sql-pg`); `src/push.ts`'s one-shot copy still opens its own `pg.Pool` |
...
Every unbounded network read in the collector layer is capped, so one oversized or hostile response logs and gets skipped instead of taking the ingest process out with an allocation failure. `src/http.ts` exports `MAX_RESPONSE_BYTES` (32MB), the compressed/wire-size ceiling every collector's `getBytes` call shares by default (`gdelt`, `camara`, `senado`, `gkg.ts`'s zip fetch and `rss.ts`'s feed fetch, and, through it, `gnews`/`juridico`/`oficial`/`nicho`, which reuse `fetchFeed`). Each of them checks a declared `content-length` before reading a single byte of the body, then keeps a running total while streaming so a response with no (or an understated) `content-length` is still caught, without ever resolving a partial body. `gkg.ts` additionally caps the *decompressed* CSV at `MAX_EXPANDED_BYTES` (128MB), enforced while streaming through fflate's `Unzip` — the only collector that unzips anything. All three figures are literals, not environment-overridable: they are safety ceilings picked from measured corpus sizes with headroom, not an operational knob like `GKG_SLOTS`. An oversized GKG slot is skipped without being marked done in `gkg_files`, so the next `gkg()` run retries it — it either succeeds once the size problem clears or ages out of the `GKG_SLOTS` look-back window, exactly like a slot GDELT never published. A `getBytes` caller hitting the cap sees a typed `ResponseTooLarge` rejection; `gdelt`/`camara`/`senado` already log and skip per person, `rss`/`juridico`/`oficial`/`nicho` fail that one feed's whole fetch, which already fails its collector's run.

Every collector runs on Effect's fetch-backed `HttpClient` (`src/http.ts`, `fetchClient`): `getBytes` is the one capped, timed GET every collector's request goes through, so every collector's network read is timeout-bounded, not only `gdelt`/`camara`/`senado`/`bluesky`: this layer covers `rss` and `gkg` too, which had no ceiling before it. The 45 s ceiling (`REQUEST_TIMEOUT_MS`) is `getBytes`'s default third argument, and covers the whole request, first byte sent to last byte read, where a per-socket idle timer would instead wait through a body that merely trickles. `gkg.ts`'s zip download is the one exception: a slot measures 13-14MB compressed, too large for a 45 s ceiling sized for small JSON endpoints, so `download` passes its own `GKG_DOWNLOAD_TIMEOUT_MS` (300 000 ms, 5 min) as `getBytes`'s third argument instead — a ceiling distinct from both `REQUEST_TIMEOUT_MS` and the `MAX_RESPONSE_BYTES`/`MAX_EXPANDED_BYTES` byte caps, all three enforced independently. Effect's client also propagates trace context (`traceparent`, `b3`) by default; `fetchClient` turns it off, so the only request header is still the project `user-agent` (`test/http.test.ts` pins the exact header set). `src/collectors/index.ts` registers each module's `collect` Effect directly as the `Collector` the registry holds; nothing in `src/` turns a collector Effect into a Promise, `ingest.ts` runs them on `fetchClient` and the database layer at its own entrypoint. `test/effect.ts`'s `fakeFetch`/`runTest`/`drain`/`TestClock`/`TestConsole` drive `collect` directly in every collector suite.
...
`pnpm aggregate` (`src/aggregate.ts`) folds that scan into two tables the route reads by primary key:
...
The build runs at the end of `pnpm ingest` and `pnpm reindex`, both of which own `DATA_DIR`, and in `warm.yml` as `pnpm aggregate --if-missing`, which builds only when either persisted table is empty (the first production deploy after the tables appeared, or a `graph_terms` emptied under a full `graph_scopes` — on 2026-09-18 a scopes-only check called that state built and left the site blank until the next ingest) and shares ingest's concurrency group so two builds never run on one database. One transaction per window (`delete` then `insert` for `graph_scopes`/`graph_terms`, a `create temp table` for `graph_terms_all` keyed on `(days, source, term, kind)` before any person reads it, readers see the previous build until commit), one statement for the universe, one for the scopes, one per person for the terms. It is idempotent. After `pnpm push` run `pnpm aggregate` against the same `DATABASE_URL`, or the deployment answers from the live scan until the next scheduled ingest. The persisted tables come from `pnpm migrate`, and `graphFastQuery` needs them to exist: run `pnpm migrate` against production before the deploy that ships this, as with any schema change.
...
`term_communities` (`days, source, person_id, term, kind` primary key) is a third table, written last inside the same window transaction, after that window's `personTermsQuery` calls: for every `(days, source, person)` `graph_terms` was just filled for, `src/aggregate.ts` re-derives the co-occurrence edges over exactly that scope's kept `(term, kind)` rows (the same shape as `linksQuery`, joined against `graph_terms` instead of a request's live `ids`) and hands them to `src/communities.ts`'s pure `communities(edges, seed)` (`graphology` + `graphology-communities-louvain`, both pinned exact), a fixed seed so one library version always partitions one edge list the same way. Every kept term gets a row, including a term with no surviving edge (its own singleton community) — `community` is `null` in `GET /graph?communities=1` only when the whole recorte is unbuilt (the live path) or the community step has not yet covered a built window, never for an isolated word inside a build that ran it. `term_communities` is in `ANALYZED_TABLES` (`src/db.ts`) and in `POST_BUILD_ANALYZED` (`AGGREGATE_TABLES` plus itself), the list ingest and reindex analyze right after `buildGraphAggregates` returns, so its statistics describe the build that just ran rather than the previous one; it is deliberately **not** in `AGGREGATE_TABLES` and does not gate `hasGraphAggregates()`: a community build lagging behind `graph_terms`/`graph_scopes` must never look like a missing aggregate build to `pnpm aggregate --if-missing`, or that flag would re-run the whole expensive window build just to catch communities up. A stale or missing community build degrades to `null`, the same documented value the live path already returns.

## Warming the CDN

A deployment empties Vercel's cache, and the first reader of every recorte pays the origin. `pnpm warm` (`src/warm.ts`) asks the site for the recortes the page requests by itself — `/api/people`, then `graph`, `sources` and `testimony` for every person in `seed.json` at 7 and 30 days, with `atlas.html`'s defaults (`sort=pmi`, `limit=18`, `source=all`) — built with the same functions `src/ui/api.ts` uses, so the querystring, and therefore the cache key, is the page's own. The CDN keeps `s-maxage=6h` + `stale-while-revalidate=24h` and the ingest cron is 6 h, so a plain GET after an ingest would HIT, or stale-serve, the pre-ingest payload and leave the origin untouched — and no request header changes that: `Pragma: no-cache` and `Cache-Control: no-cache` were both measured as HITs on 2026-09-14. So every cacheable `/api` response carries `Vercel-Cache-Tag: api` (`CACHE_TAG`, `src/cache.ts`), and both workflows run `vercel cache dangerously-delete --tag api` right before `pnpm warm`: the stored API objects are gone, the warm's GETs are `MISS`es that fill fresh copies, and `public/` stays cached. The delete needs a `VERCEL_TOKEN` repository secret (an account token from vercel.com/account/tokens with access to the `rashomon` project; the org and project ids are in the workflow, not secret); the workflow stops if it is missing. A warm that reports mostly `HIT` is therefore a warm that did nothing. Two lanes, never a burst; it prints status and `x-vercel-cache` counts, p50/p95 and the five slowest paths with their `server-timing`, and exits non-zero on a 5xx or a failed fetch. `SITE_URL` overrides the target (default `https://rashomon-five.vercel.app`). `.github/workflows/ingest.yml` runs it after every ingest, and `.github/workflows/warm.yml` after every successful production deployment (`deployment_status`) or by hand (`gh workflow run warm.yml`), after `pnpm aggregate --if-missing` so a fresh database is never warmed through the live scan.
