...
Only the `graph` statement is precomputed. With `testimony=1`, which the page always sends, `graphFor` still runs `linksQuery` and `termTestimonyQuery` live (0.27 s + 0.03 s measured on production on 2026-09-14 with a warm cache, 0.04 s for `links` since issue #163, see [What a slow `/graph` is made of](#what-a-slow-graph-is-made-of)), so a `/graph` MISS goes from about 7 s to under half a second, not to zero. `/api/compare` (figure 3) has no aggregate at all.
