...
`gkg` downloads GDELT's 15-minute translation GKG files (`*.translation.gkg.csv.zip`), keeps Portuguese rows, uses the original page title as text and the `persons` column to match tracked people. Processed slots are recorded in `gkg_files`, so each run only fetches new ones. `GKG_SLOTS` sets how many recent slots to look back (default 24 = 6h; measured 2026-09-10, one slot was 13.6MB compressed / 43.2MB decompressed). A slot is marked done before its docs are inserted; if the insert fails you must delete the row from `gkg_files` to retry it. The download is bounded on three independent ends: the compressed zip is capped at `MAX_RESPONSE_BYTES` (32MB, shared with every other collector's `getBytes` call — see [operations](operations.md)), checked against a declared `content-length` before any byte is read and again while streaming if none was declared; the decompressed CSV is capped separately at `MAX_EXPANDED_BYTES` (128MB), enforced while unzipping so a decompression-bomb shape is caught mid-stream; and the download itself carries its own wall-clock ceiling, `GKG_DOWNLOAD_TIMEOUT_MS` (300 000 ms, 5 min) — distinct from `REQUEST_TIMEOUT_MS` (45 s), the ceiling every other collector's request carries, since a slot's 13-14MB takes longer than the small JSON endpoints that literal was sized for. Against the measured slot size that leaves about 2.3x of headroom on the compressed ceiling and about 3x on the decompressed one. The decompressed ceiling is approximate: the compressed bytes are fed to the unzipper in 16KB slices and the running total is only checked between slices, so a bomb-shaped entry can overshoot `MAX_EXPANDED_BYTES` by up to about 16MB before the loop stops feeding it. Any of the three ceilings being crossed skips the slot without marking it done in `gkg_files`, so a later run retries it — the same fate as a slot GDELT never published, and it either eventually succeeds or ages out of the `GKG_SLOTS` look-back window.

`gdelt` (DOC API) enforces ~1 request / 5s per IP and returns 429 aggressively. Off by default. A 429 is retried up to 3 times, waiting `rateLimitMs * 4 * attempt` (5500ms base, so 22s/44s/66s) between attempts; a non-JSON body on any other status, or the 4th 429, fails with the status and the head of the body.

`camara` and `senado` read a tracked parliamentarian's own floor-speech summaries over the last 30 days, keyed off `camaraId`/`senadoId` in `seed.json`; a person carrying neither is skipped without a request. Both are on by default: unpaced requests to either endpoint answered 200 with no rate limiting (senado 150-280ms, camara 490-730ms). `camara` still retries a 429/5xx three times with backoff, waiting `pauseMs * 4 * attempt` (1000ms base, so 4s/8s/12s) between attempts; `senado` does not. Neither has a window knob; 30 days is fixed in the collector, so a chamber in recess simply contributes nothing.
...
- `oficial` (institutional press): `camara.leg.br`'s POLITICA and ELEICOES feeds, `senado.leg.br`, `agenciabrasil.ebc.com.br`, and Planalto's own feed. Planalto's feed is RDF (RSS 1.0), not RSS 2.0: `rss.ts`'s `fetchFeed` reads items from both `rss.channel.item` (RSS 2.0) and `rdf:RDF.item` (RDF, where items sit as siblings of `channel` rather than nested inside it) and concatenates the two, so one parser covers both shapes with no per-feed detection. `toDoc` falls back to an item's `dc:date` when `pubDate` is absent, which is how Planalto's own ISO 8601 dates are read; that fallback is shared by every RSS-family collector, not `oficial`-specific.
