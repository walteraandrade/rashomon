...
On the deployed site the counts behind `pmi` come from tables `pnpm aggregate` rebuilds after every ingest, cut at the moment of that build rather than of the request; see [graph aggregates](operations.md#graph-aggregates). The live statement and the tables agree on every count. Terms are only stored for docs that mention at least one tracked person. A doc naming nobody tracked keeps its `docs` row (it still feeds `/api/candidates` and the source counts) but gets no `doc_terms` rows: they used to serve only as the PMI denominator, at two thirds of the largest table. PMI therefore compares a person's terms against the docs about tracked people in the window, not against everything collected; `stats.docs` keeps its old meaning (every doc in scope).

`pnpm purge orphan-terms` deletes the term rows a database collected before this rule and runs `vacuum full doc_terms` to return the pages to disk (plain `vacuum` only marks them reusable and leaves the file the same size). It takes an exclusive lock on `doc_terms`, so run it with the server stopped; it is a one-off per database, and `pnpm reindex` produces the same result from scratch. `pnpm purge <source>` does the same for the tables its cascade empties. `pnpm purge themes` clears GDELT's stored theme terms: it zeroes every `docs.extra_terms`, deletes every `doc_terms` row with `kind = 'theme'`, and deletes every pre-revision `kikori:q8`-style testimony row (a `kikori:<dtype>:<hash>` row survives, since it already carries a revision). It `vacuum full`s `docs` as well as the two term tables: emptying a jsonb column is an update, and an update under MVCC writes a new tuple and leaves the old one dead exactly as a delete does, so without it the largest share of the freed space stays in the file. That lock is on the biggest table, so run this one with the server stopped.
