...
import { domainSuffix, esc, fmt, kinds, label, matching, normalize, relatedTo, safeDocUrl, score, scoreName, toneColor } from './format.js'
import { routesFrom } from './layout.js'
...
export const createCanvasMeasure = () => {
  ...
  return (text, size, family = 'Inter, system-ui, sans-serif', weight = 500) => {
...
export const drawMap = ({ layout, personName, about, mode, sort, onChoose }) => {
  ...
  $('overflow').innerHTML = overflow.length
    ...
    : ''
  for (const el of document.querySelectorAll('#viewport [data-node], #overflow [data-node]')) {
    el.addEventListener('click', () => onChoose(el.dataset.node))
    if (el.tagName.toLowerCase() === 'g')
      el.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault()
          onChoose(el.dataset.node)
        }
      })
  }
  $('legend').innerHTML =
...
export const paintColumns = ({ nodes, links, selected, search, sort, mode, onChoose }) => {
  ...
  $('columns')
    ...
    .forEach((el) => el.addEventListener('click', () => onChoose(el.dataset.col)))
}
