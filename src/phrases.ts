...
// so the join can be an inner one and no pair is silently dropped for want of a unigram.
const buildSql = `
  insert into phrases (term, count, score)
  with uni as (select w1 as w, count(*)::float8 as c from phrase_stage group by 1),
  pairs as (select w1, w2, count(*)::float8 as c from phrase_stage where w2 is not null group by 1, 2)
...
export const buildPhrases = async () => {
  await db.exec(`truncate phrases`)
  await db.query(buildSql, [minPhraseCount(), minPhrasePercent() / 100])
