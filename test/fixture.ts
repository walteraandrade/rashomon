...
export const docs: RawDoc[] = [
  ...
  // docs 49-54: issue #93's /api/compare AC11 fixture (limit=1 must union both the count-ranked
  ...
  // that: bolsonaro uses it 3 times (docs 49/50/51) but lula also uses it twice too (docs
  // 52/53), so its global doc_terms count is inflated and its pmi comes out lower than the
  // count-1, exclusive words those same three bolsonaro docs also carry ("cita", "comunicado",
  // "discurso", "discute", "repete" all tie on pmi) — cita's pmi * ln(2) outranks termdiluido's
  // pmi * ln(4), so top-count (termdiluido) and top-pmi (cita) genuinely diverge for bolsonaro;
  // "cita" wins that tie only on the query's alphabetical tiebreak
  // (`order by pmi * ln(1 + count) desc, term, kind`), not because it is pmi-highest alone. All
  // six sit on a domain used nowhere else in the fixture ("testcorp.example"), so a compare
  // query scoped to that domain is a self-contained universe unaffected by "golpe" or any other
  // pinned term, and dated 3600+ days ago — past every window any other suite opens (3502, doc
  // 48) — so no pinned pmi/count/tone/stats literal shifts.
