...
describe('routeGraph / routesFrom', () => {
  ...
  it('caches the routing graph on layout.routing across calls', () => {
    ...
    const center = centerLabel(measure, 'Pessoa Central')
    // routesFrom stashes its routing graph on the layout object itself (a dynamic field with
    // no static type in a plain-JS module), so this cast is the test's own bookkeeping, not a
    // change to layout.js's public shape.
    const layout = packPass(measure, terms, center, 'count') as { routing?: unknown }
