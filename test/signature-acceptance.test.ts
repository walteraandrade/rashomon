...
describe('signature acceptance criteria (issue #6)', () => {
  ...
  it('AC1: default 30-day scope for lula returns exactly the spec-pinned reforma row', async () => {
    ...
    // issue #6's spec-pinned 0.58 becomes 1.58: docs /30-/35 (issue #5 tone fixtures)
    // widen the person-agnostic n.total from 6 to 12, shifting pmi = log2(3*12/(4*3)).
    assert.deepEqual(g.signature, [{ term: 'reforma', kind: 'word', count: 3, pmi: 1.58 }])
  })
